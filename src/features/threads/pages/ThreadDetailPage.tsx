...
import { Star, Clock, Smile, Send } from 'lucide-react'
import { formatDistanceToNow, format } from 'date-fns'
import { es } from 'date-fns/locale'
import EmojiPicker from 'emoji-picker-react'
import { type AppDispatch, type RootState } from '../../../store'
import { incrementRepliesCount, fetchTopicById, toggleStar } from '../store/threadsSlice'
import { fetchRepliesByTopic, createReply, toggleReaction, groupReactions, addReplyRealtime } from '../../posts/store/postsSlice'
            ...
            {isAuthenticated && (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1 px-2 py-0.5 rounded-full border border-slate-200 text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-colors text-sm"
                >
                  <Smile size={13} />
        ...
        <form onSubmit={handleSubmitReply} className="bg-white rounded-xl border border-slate-200 p-5 space-y-3">
          ...
          <div className="flex justify-end">
            <button
              ...
              className="flex items-center gap-2 bg-indigo-600 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors disabled:opacity-50"
            >
