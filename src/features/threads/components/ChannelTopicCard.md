...
**Ruta:** `src/features/threads/components/ChannelTopicCard.tsx`
**Tipo:** contenedor
**Estado:** estable

## Qué hace
Tarjeta de tema dentro de un canal con acciones de star, cerrar, pin, editar, borrar y vista previa de replies. Carga replies bajo demanda.
...
| --- | --- | --- | --- | --- |
| `topic` | `any` | Sí | N/A | Ver uso en el componente; no hay documentación inline adicional. |
...
- `showEditModal`: estado local inicializado con `false`; se invalida por interacción del usuario, cierre del flujo o recarga de datos.
...
- `src\features\threads\pages\ThreadsPage.tsx:108:                <ChannelTopicCard key={topic.id} topic={topic} maxTags={maxTags} />`
- `src\features\threads\pages\ThreadsPage.tsx:120:            <ChannelTopicCard key={topic.id} topic={topic} maxTags={maxTags} />`
