...
const ReplyBottomSheet = ({ open, onClose, onSubmit, replyingTo, submitting, topicRules = [], onOpenRules }: ReplyBottomSheetProps) => {
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-slate-800 rounded-t-2xl shadow-2xl flex flex-col" style={{ maxHeight: '85vh' }}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-700">
          ...
          <button onClick={handleClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:cursor-pointer transition-colors p-1">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-4 py-3 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-2">
          <button onClick={handleClose} className="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 px-4 py-2 rounded-lg transition-colors hover:cursor-pointer">
            Cancelar
          </button>
          <button
