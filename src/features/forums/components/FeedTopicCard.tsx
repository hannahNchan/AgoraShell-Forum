        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <span className="text-sm font-semibold text-slate-700">{topic.author?.username}</span>
              {topic.channel && (
                <Link
                  to={`/channels/${topic.channel.id}`}
                  onClick={(e) => e.stopPropagation()}
                  className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 hover:bg-indigo-100 hover:cursor-pointer transition-colors font-medium"
                >
            ...
            <Link
              ...
              className="font-semibold text-slate-800 hover:text-indigo-600 transition-colors leading-tight block text-base"
            >
              {topic.title}
            </Link>
            ...
            <div className="flex items-center gap-4 mt-3">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-indigo-500 hover:cursor-pointer transition-colors"
              >
          ...
          {loadingReplies ? (
            ...
            <div className="space-y-3">
              {replies.map((r: any) => (
                <div key={r.id} className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0 bg-slate-50 rounded-lg px-3 py-2">
                    <span className="text-xs font-semibold text-slate-600">{r.author?.username} </span>
              ...
              {topic.replies_count > 3 && (
                <Link
                  to={`/channels/${topic.channel_id}/topics/${topic.id}`}
                  className="text-xs text-indigo-500 hover:underline hover:cursor-pointer block"
                >
